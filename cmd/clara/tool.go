...
func parseToolCallValue(raw string) (any, error) {
	...
	// Handle common escape sequences for raw string arguments from the CLI.
...
func decodeTool(data any) (toolDetails, error) {
	...
	triggers, err := decodeToolTriggers(m["triggers"])
	if err != nil {
		return toolDetails{}, err
	}
...
func decodeToolTriggers(data any) ([]toolcatalog.Trigger, error) {
	if data == nil {
		return nil, nil
	}
	...
	items, ok := data.([]any)
	if !ok {
		return nil, fmt.Errorf("unexpected triggers payload: %T", data)
	}

	triggers := make([]toolcatalog.Trigger, 0, len(items))
	for _, item := range items {
		m, ok := item.(map[string]any)
		if !ok {
			return nil, fmt.Errorf("unexpected trigger entry: %T", item)
		}
		triggers = append(triggers, toolcatalog.Trigger{
			Name:        stringValue(m["name"]),
			Description: stringValue(m["description"]),
		})
	}
