...
func runOneOff(parent context.Context, taskFile string, verbose bool) error {
	...
	intent, err := orchestrator.LoadIntentFile(taskFile, data, namespaces)
	if err != nil {
	...
	// Wait for MCP servers to be ready before proceeding with one-off run.
	waitCtx, waitCancel := context.WithTimeout(ctx, cfg.MCPStartupTimeout)
	defer waitCancel()
	if err := reg.WaitReady(waitCtx); err != nil {
		if errors.Is(err, context.DeadlineExceeded) {
			logger.Warn().
				Dur("timeout", cfg.MCPStartupTimeout).
				Msg("MCP servers did not become ready in time; proceeding with available tools")
		} else if !errors.Is(err, context.Canceled) {
			return errors.Wrap(err, "wait for MCP servers")
		}
	}
