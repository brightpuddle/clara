...
// GetStatus returns live status information for all Clara components.
...
func (c *Client) UpdateReminder(ctx context.Context, id, title, notes string, dueDate *timestamppb.Timestamp) error {
req := &agentv1.UpdateReminderRequest{Id: id, Title: title, Notes: notes}
if dueDate != nil {
req.DueDate = dueDate
}
_, err := c.agent.UpdateReminder(ctx, req)
return errors.Wrap(err, "update reminder")
}
