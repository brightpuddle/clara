...
func (a *Agent) Run(ctx context.Context) error {
	...
	if runtime.GOOS == "darwin" && a.cfg.NativeWorkerPath != "" {
		// Wait a bit for the worker to start and create its socket.
		for i := 0; i < 10; i++ {
...
func (a *Agent) startNativeWorker(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			return
		default:
			if _, err := os.Stat(a.cfg.NativeWorkerPath); err != nil {
				a.logger.Error().Err(err).Str("path", a.cfg.NativeWorkerPath).Msg("native worker binary not found")
				return
			}

			// Clean up any stale socket before starting.
			_ = os.Remove(a.cfg.NativeSocketPath())

			cmd := exec.CommandContext(ctx, a.cfg.NativeWorkerPath)
			cmd.Stdout = os.Stdout
			cmd.Stderr = os.Stderr

			a.logger.Info().Str("path", a.cfg.NativeWorkerPath).Msg("starting native worker")
			if err := cmd.Run(); err != nil {
				if ctx.Err() != nil {
					return // Shutdown triggered.
				}
				a.logger.Error().Err(err).Msg("native worker crashed; restarting in 5s")
