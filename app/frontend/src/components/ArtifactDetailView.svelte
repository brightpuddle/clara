...
>
  {#if artifact}
    <!-- Toolbar -->
    <div
      class="flex items-center gap-2 px-4 py-2 border-b border-base-300/30 shrink-0"
    >
      <span
        class="text-xs font-medium uppercase tracking-wide text-base-content/40"
      >
        {artifact.kind}
      </span>
      <span class="flex-1" />
    ...
    >
      ...
      <div class="flex flex-wrap gap-3 text-xs text-base-content/50">
        ...
        {#if artifact.source_path}
          <span title={artifact.source_path} class="flex items-center gap-1">
        ...
        {#if artifact.source_app}
          <span class="flex items-center gap-1">
            <Icon icon="mdi:link-variant" class="w-3.5 h-3.5" />
            {artifact.source_app}
          </span>
        {/if}
        {#if artifact.due_at}
      ...
      {#if artifact.content}
        ...
        <pre
          class="whitespace-pre-wrap text-sm text-base-content/80 font-sans leading-relaxed">
{artifact.content}</pre>
  ...
  {:else}
    <div
      class="flex-1 flex items-center justify-center text-base-content/25 text-sm select-none"
    >
      Select an item
    </div>
  {/if}
