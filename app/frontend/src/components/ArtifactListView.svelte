<script lang="ts">
  ...
  import { visibleArtifacts, selectedId, focusPane, searchQuery, selectedDetail, moveSelection, jumpTo } from '../lib/store'
  ...
  $: if ($selectedId) {
    getArtifact($selectedId).then((d) => selectedDetail.set(d)).catch(() => {})
  } else {
  ...
  function handleKey(e: KeyboardEvent) {
    ...
    switch (e.key) {
      ...
      case 'k': case 'ArrowUp':
        e.preventDefault(); moveSelection(-1); break
      case 'g':
        if ((e as any)._gg) { e.preventDefault(); jumpTo('top') }
        else {
          ;(e as any)._gg = true
          setTimeout(() => { delete (e as any)._gg }, 300)
        }
        break
      case 'G':
        e.preventDefault(); jumpTo('bottom'); break
      case '/':
        e.preventDefault()
        searching = true
  ...
  <div class="px-3 py-2 border-b border-base-300/30">
    {#if searching}
      <div class="flex gap-1">
        <input
          ...
          on:keydown={(e) => { if (e.key === 'Escape') clearSearch(); if (e.key === 'Enter') runSearch() }}
    ...
    {:else}
      <button
        ...
        on:click={() => { searching = true; tick().then(() => searchInput?.focus()) }}
      >
        🔍 Search…
      </button>
    {/if}
    ...
    {#each $visibleArtifacts as artifact (artifact.id)}
      <div data-selected={artifact.id === $selectedId}>
        <ArtifactRow
          ...
          on:click={() => { selectedId.set(artifact.id); focusPane.set('list') }}
        />
      </div>
    {/each}
