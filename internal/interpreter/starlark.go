...
func (it *StarlarkInterpreter) Execute(
	...
	// Wait for MCP servers to be ready if a timeout is configured.
	if it.mcpTimeout > 0 {
		waitCtx, cancel := context.WithTimeout(ctx, it.mcpTimeout)
		if err := it.reg.WaitReady(waitCtx); err != nil {
			it.log.Warn().Err(err).Msg("MCP startup timeout expired; proceeding with available tools")
		}
		cancel()
	}
...
func (rt *starlarkRuntime) nextHistory(
	...
	if name == "tui.notify.send_interactive" || name == "tui.notify.send" {
		// id, run_id, and intent_id are dynamic/contextual and should be ignored during replay comparison
...
func (p *NamespaceProxy) Attr(name string) (starlark.Value, error) {
	// Sanitization: underscores in Starlark map to hyphens in MCP tools.
	// mail.search_all() -> mail.search-all
	sanitized := strings.ReplaceAll(name, "_", "-")
	fqName := p.name + "." + sanitized

	// Check if this is a known namespace (e.g. "theme", "mail", or an active server).
	// If it's NOT a known namespace, we return (nil, nil) so Starlark raises AttributeError.
	if !p.rt.reg.IsKnownNamespace(p.name) {
		return nil, nil
	}

	// If this name itself is a known namespace, return another proxy for nested calls.
	// e.g., tui.notify.send -> tui is NS, notify is sub-NS.
	if p.rt.reg.IsKnownNamespace(fqName) {
		return &NamespaceProxy{rt: p.rt, name: fqName}, nil
	}

	// We return a Builtin even if the tool is not currently registered.
	// This allows using these references in clara.on(theme.on_change)
	// even if "on_change" is an event rather than a tool.
	// The error will only be raised if the builtin is actually CALLED.
	return starlark.NewBuiltin(fqName, func(thread *starlark.Thread, b *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
		// Real-time lookup during call execution.
		actualFQName := fqName
		if !p.rt.reg.Has(actualFQName) {
			// Fallback: try with the original name (maybe it used underscores)
...
func (p *NamespaceProxy) AttrNames() []string {
	...
	for _, name := range p.rt.reg.Names() {
		if strings.HasPrefix(name, prefix) {
			...
			// Map back hyphens to underscores for Starlark compatibility.
