...
func New(vaultRoot string, indexPath string, log zerolog.Logger) (*server.MCPServer, error) {
	v, err := Open(vaultRoot, indexPath, log)
	if err != nil {
		return nil, err
	}
	...
	h := &handlers{vault: v, log: log.With().Str("component", "mcp_zk").Logger()}
...
func (h *handlers) checkIndexing() (*mcp.CallToolResult, bool) {
	if h.vault.IsIndexing() {
		return mcp.NewToolResultError("The Zettelkasten vault is currently being indexed. Please try again in a few seconds."), false
	}
	return nil, true
}
...
func (h *handlers) handleNoteCreate(
	...
	if err := h.vault.IndexPath(ctx, absPath); err != nil {
...
func (h *handlers) handleNoteUpdate(
	...
	if indexErr := h.vault.IndexPath(ctx, note.Path); indexErr != nil {
		return mcp.NewToolResultError(
			fmt.Sprintf("note_update: index: %v", indexErr),
		), nil
	}
	return mcp.NewToolResultJSON(map[string]any{"status": "success", "message": "updated"})
}
...
func (h *handlers) handleNoteDelete(
	...
	h.vault.RemovePath(ctx, note.Path)
	return mcp.NewToolResultJSON(map[string]any{"status": "success", "message": "deleted"})
}
...
func (h *handlers) handleTagNotes(
	...
	return mcp.NewToolResultJSON(infos)
}

func (h *handlers) handleNoteSearch(
	ctx context.Context,
	req mcp.CallToolRequest,
) (*mcp.CallToolResult, error) {
	if res, ok := h.checkIndexing(); !ok {
	...
	notes := h.vault.Search(ctx, query, limit)
	sort.Slice(notes, func(i, j int) bool {
		return notes[i].Path < notes[j].Path
	})
	infos := make([]noteInfo, len(notes))
...
func (h *handlers) handleVaultReload(
	...
	if h.vault.indexer != nil {
		h.vault.indexer.Close()
	}

	v, err := Open(h.vault.root, h.vault.indexPath, h.log)
	if err != nil {
		return mcp.NewToolResultError(fmt.Sprintf("vault_reload: %v", err)), nil
	}

	h.vault = v
	return mcp.NewToolResultJSON(map[string]any{"status": "success", "message": "Vault indexing started in background."})
}
