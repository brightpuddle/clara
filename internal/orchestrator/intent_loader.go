...
func (l *starlarkIntentLoader) onBuiltin(
	...
	// Validation: Check if the name looks like a namespaced tool (e.g. "theme.on_change")
	if !strings.Contains(name, ".") {
		return nil, errors.Newf("invalid trigger %q: must be a namespaced tool reference (e.g. theme.on_change)", name)
	}

	// Reverse the sanitization done by NamespaceProxy for event triggers,
	// because event methods usually retain their underscores (e.g. "wait_for_change").
	// We restore underscores, though this means tools with actual hyphens used as triggers
	// might be mapped to underscores. This is a known limitation of using the tool reference.
