...
// engine: Blueprint, State, and Transition.
...
// Blueprint is a declarative state machine that the interpreter executes.
...
// Validate returns an error if the Blueprint is structurally invalid.
...
// State is a single node in the Blueprint execution graph.
type State struct {
...
// ValidationError is returned when a Blueprint fails structural validation.
...
func (e *ValidationError) Error() string {
	return "invalid blueprint field " + e.Field + ": " + e.Message
}

// ParseBlueprint deserialises a JSON document into a Blueprint and validates it.
