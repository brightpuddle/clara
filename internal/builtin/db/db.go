...
func (s *service) handleQuery(ctx context.Context, args map[string]any) (any, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "db.query columns")
	}
	...
	for rows.Next() {
		...
		if err := rows.Scan(scanArgs...); err != nil {
			return nil, errors.Wrap(err, "db.query scan")
		}
...
func (s *service) handleExec(ctx context.Context, args map[string]any) (any, error) {
	...
	if err != nil {
		return nil, errors.Wrap(err, "db.exec affected")
	}
...
func (s *service) handleStageRows(ctx context.Context, args map[string]any) (any, error) {
	...
	replace, ok := args["replace"].(bool)
	if !ok {
		replace = true
	}

	if !sqliteIdentifierPattern.MatchString(table) {
		return nil, errors.Newf("invalid table name: %q", table)
	}

	tx, err := s.db.BeginTx(ctx, nil)
	if err != nil {
		return nil, errors.Wrap(err, "db.stage_rows begin")
	}
	defer tx.Rollback()

	if replace {
		if _, err := tx.ExecContext(ctx, fmt.Sprintf("DROP TABLE IF EXISTS %s", table)); err != nil {
			return nil, errors.Wrap(err, "db.stage_rows drop")
		}
	}

	if _, err := tx.ExecContext(ctx, fmt.Sprintf("CREATE TABLE IF NOT EXISTS %s (data JSON)", table)); err != nil {
		return nil, errors.Wrap(err, "db.stage_rows create")
	}

	stmt, err := tx.PrepareContext(ctx, fmt.Sprintf("INSERT INTO %s (data) VALUES (?)", table))
	if err != nil {
		return nil, errors.Wrap(err, "db.stage_rows prepare")
	}
	defer stmt.Close()
	...
	for _, row := range rows {
		...
		if err != nil {
			return nil, errors.Wrap(err, "db.stage_rows marshal")
		}
		if _, err := stmt.ExecContext(ctx, string(b)); err != nil {
			return nil, errors.Wrap(err, "db.stage_rows insert")
		}
	}

	if err := tx.Commit(); err != nil {
		return nil, errors.Wrap(err, "db.stage_rows commit")
	}
...
func encodeVector(v any) ([]byte, error) {
	...
	case []float32:
		...
		for i, f := range val {
			...
			res[i*4] = byte(bits)
			res[i*4+1] = byte(bits >> 8)
			res[i*4+2] = byte(bits >> 16)
			res[i*4+3] = byte(bits >> 24)
		}
		return res, nil
	case nil:
